...
Boot-time cost: opening three SQLite files instead of one adds an analytic upper-bound estimate of ~150 ms cold-boot (3 × ADR-0003's referenced ~50 ms single-file fsstore cold-open, cold-cache; warm-cache boot is expected lower because the OS page cache is shared across files). `BenchmarkBootTime_ThreeSQLiteFiles` will pin the real number before the estimate is removed. The cmd shell stays inside ADR-0003's `< 500 ms` cold-boot budget on the estimate.
