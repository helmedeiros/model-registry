...
|---|---|---|
| `Put` small artifact (~10 KB) | < 15 ms | SHA-256 of 10 KB (~25 µs) + payload file `fsync` (~3 ms on NVMe SSD, ~10 ms on gp2 EBS) + SQLite single-row INSERT + WAL `COMMIT` `fsync` (~1–3 ms). Expected ~4–6 ms on SSD. Bar widened from the gut-feel 5 ms once the WAL commit cost was accounted for; a 5 ms bar would have been unmet on any durable configuration. Operators that prefer the tighter bar can run `_synchronous=NORMAL` (WAL frame fsync skipped per commit, durability degraded to "no data loss on app crash" rather than "no data loss on power loss"); the substrate ships `FULL` and lets operators tune it explicitly. |
...
| `GetBundle` (metadata only) | < 5 ms | SQLite SELECT against the artifacts row (~1 ms warm-cache; 1–5 ms cold) + JSON decode of metadata struct (microseconds). No file reads — body bytes are not loaded by `GetBundle`. Expected ~1–2 ms warm; ~3–5 ms cold. |
| `GetMember(MemberSource)` (fast path) | < 5 ms | SQLite SELECT for `content_type` (~1 ms warm) + single `open + read + close` of the source file. The SELECT can be avoided by reading `content_type` from `metadata.json` on disk; that optimization is open to the implementation. Expected ~1–2 ms for sources ≤ ~100 KB on a warm page cache. For multi-MB sources on cold gp2 EBS, the file read alone can reach 10–50 ms — that regime is covered by the `Put` large tier callout above and a separate `BenchmarkStoreGet_LargeSource` row will pre-register against measured hardware in `scientific/v0.0.1/REPORT.md`. |
