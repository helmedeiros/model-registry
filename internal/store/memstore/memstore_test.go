...
func newStoreAt(start time.Time) (*memstore.Store, *fakeClock) {
...
func putCSV(t *testing.T, s *memstore.Store, name string) store.Hash {
	t.Helper()
...
func TestPutIsIdempotentOnSameSourceBytes(t *testing.T) {
	s, _ := newStoreAt(time.Unix(0, 0))
...
func TestGetBundleReturnsMetadataOnlyWithMemberFlags(t *testing.T) {
	s, _ := newStoreAt(time.Unix(0, 0))
	h, err := s.Put(ctx(), store.PutRequest{
...
func TestGetMemberDispatchesByMemberKind(t *testing.T) {
	s, _ := newStoreAt(time.Unix(0, 0))
...
func TestGetMemberAbsentForUnuploadedDerivedMember(t *testing.T) {
	s, _ := newStoreAt(time.Unix(0, 0))
...
func TestGetMemberRejectsUnknownMemberKind(t *testing.T) {
	s, _ := newStoreAt(time.Unix(0, 0))
...
func TestTagRePointingMovesHead(t *testing.T) {
	s, _ := newStoreAt(time.Unix(0, 0))
	h1 := putCSV(t, s, "v1")
	h2 := putCSV(t, s, "v2")
...
func TestTagDeprecatedArtifactInvalidTransition(t *testing.T) {
	s, _ := newStoreAt(time.Unix(0, 0))
	h := putCSV(t, s, "alpha")
...
func TestResolveTagUnknownReturnsErr(t *testing.T) {
	s, _ := newStoreAt(time.Unix(0, 0))
...
func TestListTagsReturnsCurrentHeads(t *testing.T) {
	s, _ := newStoreAt(time.Unix(0, 0))
	h1 := putCSV(t, s, "first")
...
func TestListAppliesStateFilter(t *testing.T) {
	s, _ := newStoreAt(time.Unix(0, 0))
...
func TestListCursorUnknownStartsFromBeginning(t *testing.T) {
	// Silent fall-through on unknown cursor is the agreed contract:
	// future backings (fsstore, etc.) must match, or the shared
	// conformance suite will catch the divergence.
	s, _ := newStoreAt(time.Unix(0, 0))
	putCSV(t, s, "alpha")
...
func TestListTieBreaksByHashWhenCreatedAtEqual(t *testing.T) {
	s, _ := newStoreAt(time.Unix(0, 0))
	...
	// ADR-0002 tie-break: ascending hash order when created_at is equal.
	low, high := h1, h2
	if h2 < h1 {
		low, high = h2, h1
	}
...
func TestPutPreservesByteIndependenceFromInputSlices(t *testing.T) {
	s, _ := newStoreAt(time.Unix(0, 0))
